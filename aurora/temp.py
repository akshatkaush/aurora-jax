...
#             hPa.
#         rollout_step (int, optional): How many roll-out steps were used
#            to produce this prediction.
...
#         children = (self.lat, self.lon, self.time, self.atmos_levels, self.rollout_step)
#         aux_data = None
...
# @struct.dataclass(frozen=True)
# class Batch:
#     surf_vars: dict[str, jnp.ndarray] = struct.field(pytree_node=True)
#     static_vars: dict[str, jnp.ndarray] = struct.field(pytree_node=True)
#     atmos_vars: dict[str, jnp.ndarray] = struct.field(pytree_node=True)
#     metadata: Metadata = struct.field(pytree_node=False)
...
#             },
#             static_vars={
#                 k: unnormalise_surf_var(v, k, stats=surf_stats) for k, v
#                                                       in self.static_vars.items()
#             },
#             atmos_vars={
...
#     def tree_flatten(self):
#         children = (self.surf_vars, self.static_vars, self.atmos_vars, self.metadata)
#         aux_data = None
#         return children, aux_data
...
# if not globals().get("_PYTREE_REGISTERED", False):
#     try:
#         jax.tree_util.register_pytree_node(Metadata, Metadata.tree_flatten,
#                                       Metadata.tree_unflatten)
#     except ValueError:
#         # Already registered, skip
#         pass

#     try:
#         jax.tree_util.register_pytree_node(Batch, Batch.tree_flatten, Batch.tree_unflatten)
#     except ValueError:
#         # Already registered, skip
#         pass
