...
from aurora.model.checkpoint_loader import load_pytorch_checkpoint
...
params = load_pytorch_checkpoint("microsoft/aurora", "aurora-0.25-small-pretrained.ckpt")

params = {key: value for key, value in params.items() if key.startswith("encoder")}
# Filter template_params to only encoder for consistent comparison
encoder_template_params = {"encoder": template_params["encoder"]} if "encoder" in template_params else {}

print("JAX parameters (encoder only):")
for key, (shape, dtype) in extract_jax_params(encoder_template_params).items():
    print(f"Path: {key}, Shape: {shape}, dtype: {dtype}")

print("\nPyTorch parameters (encoder only):")
